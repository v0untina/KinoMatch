...
import { getPublishedCompilations, type UserCompilationSummary } from '@/api/compilations';
...
// --- PostCard компонент ---
// (Полный код PostCard, как был ранее)
const PostCard = ({ post }: { post: Post }) => {
    const [isLiked, setIsLiked] = useState(false); // Начальное состояние - TODO: получать с сервера
    ...
    const handleSubmitComment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setCommentText(''); setShowCommentsModal(false);
        } catch (error: any) {
    ...
    return (
        <div className={`${styles.post} ${styles.roundedCard}`}>
            ...
            {showCommentsModal && ( <div className={styles.modal_overlay} onClick={handleCloseCommentsModal}><div className={styles.comments_modal} onClick={(e) => e.stopPropagation()}><h3>Комментарий к посту</h3><form onSubmit={handleSubmitComment} className={styles.comment_form}><textarea className={styles.modal_textarea} placeholder="Напишите ваш комментарий..." value={commentText} onChange={(e) => setCommentText(e.target.value)} rows={4} required disabled={isSubmittingComment}/><button type="submit" className={styles.submit_comment_button} disabled={isSubmittingComment || !commentText.trim()}>{isSubmittingComment ? 'Отправка...' : 'Отправить'}</button></form><button className={styles.modal_close} onClick={handleCloseCommentsModal} disabled={isSubmittingComment} aria-label="Закрыть">×</button></div></div>)}
            {/* Основной контент поста */}
            <div className={styles.postMainContent}>
                <div className={styles.postHeader}><img className={styles.postUserAvatar} src={post.userAvatar || '/interface/defaultAvatar.webp'} alt={`${post.username || 'User'} avatar`} onError={handleAvatarError}/><div className={styles.postUserInfo}><span className={styles.postUsername}>{post.username || 'Автор неизвестен'}</span><span className={styles.postTimestamp}>{formattedDate}</span></div></div>
                <div className={styles.postContent}>{post.content && <p>{post.content}</p>}{post.imageUrl && (<img className={styles.postImage} src={post.imageUrl} alt="Изображение к посту" onError={handleImageError} loading="lazy"/>)}</div>
                <div className={styles.postActions}>
                    <button onClick={handleLikeClick} className={`${styles.actionButton} ${isLiked ? styles.liked : ''}`} disabled={isLiking} aria-pressed={isLiked}><img src={isLiked ? '/interface/like_ON.png' : '/interface/like_OFF.png'} alt={isLiked ? 'Убрать лайк' : 'Поставить лайк'} className={styles.actionIcon}/><span className={styles.likesCounter}>{likesCount}</span></button>
                    <button onClick={handleCommentClick} className={styles.actionButton} disabled={isSubmittingComment}><img src="/interface/commenting.png" alt="Комментировать" className={styles.actionIcon}/><span className={styles.commentsCounter}>{displayComments.length}</span></button>
                </div>
            </div>
             {/* Блок комментариев */}
            <div className={`${styles.comments} ${styles.roundedCard}`}>
                <span className={styles.comments_title}>Комментарии</span>
                <div className={styles.reviews}>{(!displayComments || displayComments.length === 0) ? (<p className={styles.comment_placeholder}>Комментариев пока нет.</p>) : (displayComments.map((comment) => (<div key={comment.id} className={styles.review}><img className={styles.image_user} src={'/interface/defaultAvatar.webp'} alt={`${comment.authorUsername} avatar`} onError={handleAvatarError}/><div className={styles.comment_content_container}><div className={styles.comment_header}><span className={styles.comment_author}>{comment.authorUsername}</span><span className={styles.comment_timestamp}>{comment.createdAt ? new Date(comment.createdAt).toLocaleString('ru-RU', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</span></div><p className={styles.comment}>{comment.text}</p></div></div>)))}</div>
            </div>
        </div>
    );
};
...
// --- NewsCard компонент ---
// (Полный код NewsCard, как был ранее)
const NewsCard = ({ newsItem }: { newsItem: NewsItem }) => {
    const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => { e.currentTarget.style.display = 'none'; };
    return (
        <div className={`${styles.newsCard} ${styles.roundedCard}`}>
            {newsItem.images && newsItem.images.length > 0 && (<a href={newsItem.link || '#'} target="_blank" rel="noopener noreferrer"><img src={newsItem.images[0]} alt={newsItem.title} className={styles.newsImage} onError={handleImageError} loading="lazy"/></a>)}
            <div className={styles.newsContent}>
                <h3><a href={newsItem.link || '#'} target="_blank" rel="noopener noreferrer" className={styles.newsTitleLink}>{newsItem.title}</a></h3>
                {newsItem.text && <p className={styles.newsText}>{newsItem.text.substring(0, 200)}{newsItem.text.length > 200 ? '...' : ''}</p>}
                <div className={styles.newsMeta}><div className={styles.newsCategory}>{newsItem.category || 'Без категории'}</div>{newsItem.date && <span className={styles.newsDate}>{newsItem.date}</span>}{newsItem.author && <span className={styles.newsAuthor}>Автор: {newsItem.author}</span>}{newsItem.views && <span className={styles.newsViews}>Просмотры: {newsItem.views}</span>}</div>
...
// --- КОМПОНЕНТ КАРТОЧКИ ОПУБЛИКОВАННОЙ ПОДБОРКИ ---
// (Полный код PublishedCompilationCard, как был ранее)
interface PublishedCompilationCardProps { compilation: UserCompilationSummary; }
const PublishedCompilationCard: React.FC<PublishedCompilationCardProps> = ({ compilation }) => {
    const PUBLIC_POSTER_PATH = '/posters/'; // Убедись, что путь верен
    const PLACEHOLDER_POSTER = `${PUBLIC_POSTER_PATH}placeholder.jpg`; // Убедись, что файл есть
    ...
    return (
        <Link href={`/compilations/${compilation.id}`} className={styles.userCompilationCard} title={`Подборка "${compilation.title}"`}>
             <div className={styles.userCompilationPosters}>
                 {compilation.previewPosters.slice(0, 4).map((poster, index) => (<div key={index} className={styles.userCompilationPosterWrapper}><Image src={getPosterUrl(poster)} alt="" fill sizes="(max-width: 768px) 25vw, 100px" style={{ objectFit: 'cover' }} className={styles.userCompilationPoster} onError={handleImageError} unoptimized/></div>))}
                 {Array.from({ length: Math.max(0, 4 - compilation.previewPosters.length) }).map((_, index) => (<div key={`placeholder-${index}`} className={`${styles.userCompilationPosterWrapper} ${styles.placeholder}`}></div>))}
             </div>
             <div className={styles.userCompilationInfo}>
                 ...
                 <div className={styles.userCompilationMeta}>
                     ...
                     {compilation.author && (<span className={styles.compilationAuthor}> от {compilation.author.username}</span>)}
                 </div>
             </div>
         </Link>
    );
...
const GeneralPage = () => {
    ...
    const [publishedCompilations, setPublishedCompilations] = useState<UserCompilationSummary[]>([]);
    ...
    const loadPublishedCompilations = useCallback(async () => {
        if (publishedLoaded || isLoadingPublished) return; setIsLoadingPublished(true); setErrorPublished(null); console.log("Fetching published compilations for GeneralPage...");
        try {
            const response = await getPublishedCompilations(20, 0); setPublishedCompilations(response.data); setPublishedLoaded(true); console.log("Published compilations fetched:", response.data);
        } catch (error: any) { console.error("Failed to load published compilations:", error); const message = error.message || "Не удалось загрузить подборки."; setErrorPublished(message); toast.error(`Ошибка загрузки подборок: ${message}`); }
        finally { setIsLoadingPublished(false); }
    }, [publishedLoaded, isLoadingPublished]);

    useEffect(() => { if (activeSection === 'collections') { loadPublishedCompilations(); } }, [activeSection, loadPublishedCompilations]);
                        ...
                        {!isLoadingPublished && !errorPublished && (
                            publishedCompilations.length === 0 ? (<p>Опубликованных подборок пока нет.</p>) : (
                                <div className={styles.userCompilationsGrid}>
                                    {publishedCompilations.map((comp) => ( <PublishedCompilationCard key={comp.id} compilation={comp} /> ))}
                                </div>
                            )
                         )}
                     </div>
